...
export class CardTemplate {
  static generate(point: IPoint): string {
    const subName = point.legendSubName
      ? `<span class="data__title__container--subname"> ${point.legendSubName} </span>`
      : '';
    ...
    // Inline SVG ikona z kolorem
    const iconSvg = IconService.getColoredIcon(point.icon, point.color);
    const iconHtml = iconSvg
      ? `<div class="data__title__container--icon">${iconSvg}</div>`
      : '';
    ...
    const contactHtml =
      addressesHtml || phoneHtml || emailHtml || facebookHtml || webpageHtml
                ...
                ${point.images
                  .map(
                    (img: string, index: number) =>
                      `<li class="glide__slide" data-index="${index}"><img src="${img}" alt="Thumbnail" /></li>`
                  )
                  .join('')}
              </ul>
            </div>
      ...
      <div class="cards-container__item">
        <div class="cards-container__item__header">
          <div class="data__title__container">
            <img class="data__title__container--icon" src="${
              point.icon
            }" alt="${point.name}">
